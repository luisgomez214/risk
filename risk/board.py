...
class Board(object):
    ...
    def hostile_neighbors(self, territory_id):
        """
        Create a generator of all territories neighboring a
        given territory, of which
    ...
    def is_valid_attack_path(self, path):
        ...
        All valid attacks, therefore, will follow a path of starting
        on one player's territory and moving trough enemy territories.

        Formally, an attack path is a valid path satisfying the following
        two additional properties:
        1. An attack path must contain at least two territories;
        1. If the first territory is owned by player A, then no other
        territories in the path are also owned by A.
    ...
    def cost_of_attack_path(self, path):
        '''
        The cost of an attack path is the total number of enemy
        armies in the path.
        In other words, it is the total number of armies in the
        subpath starting at the second vertex.

        Args:
            path ([int]): a list of territory_ids which
            must be a valid attack path
    ...
    def shortest_path(self, source, target):
        ...
        This function does not take into account who owns the
        territories or how many armies are on the territories,
        ...
        Returns:
            [int]: a valid path between source and target that has
            minimum length; this path is guaranteed to exist
        '''
        path = []
        if source == target:
            path.append(source)
        ...
        while queue:
            ...
            for neighbor in self.neighbors(y):
                if neighbor.territory_id not in visited:
                    ...
                    visited[neighbor.territory_id] = y
        return []
    ...
    def can_fortify(self, source, target):
        ...
        Returns:
            bool: True if reinforcing the target from the
            source territory is a valid move
        '''
        player_id = self.owner(target)
        if self.owner(source) != player_id:
            return False
        visited = set()
        queue = set()
        queue = [source]
    ...
    def cheapest_attack_path(self, source, target):
        ...
        or there may be no valid attack paths (in which case
        ...
        while q:
            ...
            for neighbor in self.neighbors(v):
                ...
                if owner == attacker:
                    continue
                new_path = path + [territory_id]
                if self.is_valid_attack_path(new_path):
                    heapq.heappush(q, (self.cost_of_attack_path(new_path), new_path))
        return None
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        Args:
            from_territory (int): Territory_id of the territory where
            armies leave.
            to_territory (int): Territory_id of the territory where
            armies arrive.
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        Raises:
            ...
            ValueError if a player attacks himself or the territories do
            not share a border.
    ...
    def plot_board(self, path=None, plot_graph=False, filename=None):
        ...
        Args:
            ...
            filename (str): if given, the plot will be saved to
            the given filename instead of displayed
        """
